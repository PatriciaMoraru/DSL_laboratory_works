...
class Lexer:
    ...
    def tokenize_identifier_or_keyword(self):
        ...
        if text in keywords:
            self.tokens.append({"type": TokenType.KEYWORD, "value": text})
        elif text in functions:
            self.tokens.append({"type": TokenType.FUNCTION, "value": text})
        else:
            self.tokens.append({"type": TokenType.IDENTIFIER, "value": text})
    ...
    def tokenize_number(self):
        ...
        text = self.input[start:self.position]
        self.tokens.append({"type": TokenType.NUMBER, "value": text})
    ...
    def tokenize_string(self):
        ...
        while not self.is_at_end() and self.peek() != '"':
            if self.peek() == '\n':  # If a newline appears, stop parsing
                ...
                self.tokens.append({"type": TokenType.ERROR, "value": "Unterminated string"})
        ...
        for part in parts:
            ...
            if part in {"+", "-", "*", "/"}:
                self.tokens.append({"type": TokenType.OPERATOR, "value": part})
            elif part:  # Avoid empty strings
                self.tokens.append({"type": TokenType.STRING, "value": part})

    def tokenize_symbol(self):
        ch = self.advance()
        if ch in {">", "<", "=", "!"} and self.peek() == "=":
            op = ch + self.advance()
            self.tokens.append({"type": TokenType.OPERATOR, "value": op})
            return
